...
let portalVersion = 1;
let dispatchState = [
  { id: 'D-1042', code: '10-50', title: 'Közúti baleset', location: 'Vespucci Boulevard', priority: 'critical', service: 'LSPD / EMS', units: ['ADAM-12'], status: 'active', createdAt: new Date().toISOString() },
  { id: 'D-1041', code: '10-66', title: 'Gyanús jármű', location: 'Mirror Park', priority: 'high', service: 'LSPD', units: [], status: 'open', createdAt: new Date(Date.now() - 7 * 60000).toISOString() },
  { id: 'D-1039', code: 'MED', title: 'Sérült civil', location: 'Legion Square', priority: 'normal', service: 'EMS', units: ['MEDIC-3'], status: 'active', createdAt: new Date(Date.now() - 14 * 60000).toISOString() }
];
const publicFiles = new Set(['/index.html', '/map.html', '/user.html', '/style.css', '/script.js', '/road-map.js', '/map3d.js', '/Logo.png', '/assets/js/portal-bridge.js']);
const publicPrefixes = ['/assets/', '/items-images-main/', '/zerodream_3dmap-main/html/'];
...
function portalSnapshot() {
  ...
  return {
    ...
    community: { online: Math.max(players, 32), posts: 24, unread: 5, activity: 87 }
  };
}
